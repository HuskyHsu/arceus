...
def get_items(soup):
    result = soup.select(
        'table tr td a[href="/wiki/%E5%AE%9D%E5%8F%AF%E6%A2%A6%E4%BC%A0%E8%AF%B4_%E9%98%BF%E5%B0%94%E5%AE%99%E6%96%AF"]'
    )  # , title="寶可夢傳說 阿爾宙斯"
    ...
    for r in result:
        ...
        if source == "獲得方式":
            ...
            items += [
                [name] if i == "" else [name, i] for name, i in zip(info, sub_info)
            ]
...
def get_leveling_up(soup):
    result = soup.select(
        'h4 a[href="/wiki/%E5%AE%9D%E5%8F%AF%E6%A2%A6%E4%BC%A0%E8%AF%B4_%E9%98%BF%E5%B0%94%E5%AE%99%E6%96%AF"]'
    )  # , title="寶可夢傳說 阿爾宙斯"
    hash_map = {
        "進化": -1,
        "—": 0,
    ...
    for r in result:
        trs = r.parent.parent.find_next_sibling("table").select(
            'tr[class="at-c bgwhite"]'
        )
        for tr in trs:
            learn, mastery, name = [
                td.text.strip().replace("[詳]", "") for td in tr.select("td")[:3]
            ]
            move.append(
                (
...
def get_wiki_info(name):
    ...
    response = requests.get(
        f"https://wiki.52poke.com/wiki/{name}",
